...
bm_path <- "camels-spat/bm_outputs_no_monthly/"
...
# define output file
output_file <- file.path("camels-spat/outputs", "nse_no_monthly_skill_scores.csv")  # change folder if needed
...
# Find the models with the best Cal NSE
best_models <- mydf %>%
  group_by(Codes) %>%
  filter(Cal...NSE....1 == max(Cal...NSE....1, na.rm = TRUE)) %>%
  slice(1) %>%
  ungroup()
