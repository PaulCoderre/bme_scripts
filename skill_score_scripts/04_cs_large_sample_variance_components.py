...
# ====================
# This script reads the benchmark flows csv to calculate the variance components of observed streamflow at each CAMELS-SPAT
# gauge. It outputs the decomposition along with the NSE of each variance component from each benchmark.
# 
# CONFIGURABLE: You can choose which period to use for variance decomposition:
#   - 'all': Use all available data (most robust - RECOMMENDED)
#   - 'calibration': Use only calibration period
#   - 'validation': Use only validation period
# ====================
...
# PERIOD FOR VARIANCE DECOMPOSITION
# Options: 'all', 'calibration', 'validation'
decomposition_period = 'all'  # RECOMMENDED: Use all data for most robust variance estimates

# Output files (includes period in filename)
output_file = os.path.join(output_directory, f'benchmark_variance_{decomposition_period}.csv')
benchmark_summary_file = os.path.join(output_directory, f'benchmark_summary_{decomposition_period}.csv')

print("="*80)
print("VARIANCE DECOMPOSITION CONFIGURATION")
print("="*80)
print(f"Decomposition period: {decomposition_period.upper()}")
print(f"Output file: {output_file}")
print(f"Summary file: {benchmark_summary_file}")
print("="*80)
...
def calculate_variance_component_nses_and_kge(obs_decomp, sim_decomp):
    ...
    results = {
        ...
        'kge_alpha': kge_overall_results['alpha'],
...
for csv_path in csv_files:
    ...
    try:
        ...
        # ====================================================================
        # FILTER DATA BASED ON DECOMPOSITION_PERIOD FOR VARIANCE CALCULATION
        # ====================================================================
        if decomposition_period == 'calibration':
            if 'cal_mask' not in flow.columns:
                print(f"Warning: 'cal_mask' column not found in {csv_name}. Skipping.")
                continue
            n_total = len(flow)
            flow_for_decomp = flow[flow['cal_mask'] == True].copy()
            n_decomp = len(flow_for_decomp)
            print(f"Using CALIBRATION period for decomposition: {n_decomp}/{n_total} timesteps ({n_decomp/n_total*100:.1f}%)")
            
        elif decomposition_period == 'validation':
            if 'val_mask' not in flow.columns:
                print(f"Warning: 'val_mask' column not found in {csv_name}. Skipping.")
                continue
            n_total = len(flow)
            flow_for_decomp = flow[flow['val_mask'] == True].copy()
            n_decomp = len(flow_for_decomp)
            print(f"Using VALIDATION period for decomposition: {n_decomp}/{n_total} timesteps ({n_decomp/n_total*100:.1f}%)")
            
        elif decomposition_period == 'all':
            flow_for_decomp = flow.copy()
            n_decomp = len(flow_for_decomp)
            print(f"Using ALL data for decomposition: {n_decomp} timesteps")
            
        else:
            raise ValueError(f"Invalid decomposition_period: {decomposition_period}. Must be 'all', 'calibration', or 'validation'.")
        
        if n_decomp == 0:
            print(f"Warning: No data in {decomposition_period} period for {csv_name}. Skipping.")
            continue
        
        # Decompose observed flow using the selected period
        print(f"\nDecomposing observed data ({decomposition_period} period)...")
        qobs_decomp = decompose_streamflow(flow_for_decomp, 'q_obs')
        ...
        print(f"  Variance fractions: seasonal={qobs_decomp['variance_fractions']['seasonal']:.3f}, "
              f"interannual={qobs_decomp['variance_fractions']['interannual']:.3f}, "
              f"irregular={qobs_decomp['variance_fractions']['irregular']:.3f}")
        
        # Get all benchmark model columns
        benchmark_cols = [col for col in flow_for_decomp.columns if col.startswith('bm_')]
        ...
        for bm_col in benchmark_cols:
            try:
                ...
                benchmark_decomps[bm_col] = decompose_streamflow(flow_for_decomp, bm_col)
                # print(f"    Length: {len(benchmark_decomps[bm_col]['original'])} timesteps")
            except Exception as e:
                print(f"    ✗ Failed to decompose {bm_col}: {str(e)}")
                continue
        
        print(f"  Successfully decomposed {len(benchmark_decomps)} benchmarks")
...
numeric_cols = ['nse_overall', 'nse_seasonal', 'nse_interannual', 'nse_irregular',
                'kge_overall', 'kge_r', 'kge_beta', 'kge_alpha',
